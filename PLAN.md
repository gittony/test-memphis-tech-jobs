...
### Slice 11: title-ambiguous postings now get a real AI judgment call, not just an automatic fail

Triggered by a real example you found: Buckman's "Digital Innovation Engineer" (Slice 10) requires a CS degree and describes genuine software/data work, but `lib/filter.js`'s title whitelist only matches *specific* tech phrases, not a bare "engineer" — so it failed outright with no review at all, the same way it would for an actually-irrelevant title. Until now, only ambiguous *locations* got a second look from AI; an ambiguous *title* just failed.

**What changed:**
- `lib/filter.js`: a new `TITLE_MAYBE_PATTERNS` list (broader signals — `digital`, `innovation`, `technology`, `technical`, `systems`, `architect`, `automation`, `data`, `analytics`, bare `developer`, bare `IT`) sits between the strict whitelist and an outright fail. A title matching one of these but not the strict list is now `uncertain` instead of `fail`, and gets a real AI review. Deliberately **not** including a bare `/engineer/` match — that alone would send every manufacturing/process/quality engineer title at every industrial employer here to AI, a much bigger recall/cost tradeoff than this was scoped for; the narrower list already covers both motivating cases (Buckman's title via "digital," Orgill's "Sr Developer Snowflake Data Platform" via bare "developer" — a gap flagged but deliberately left unfixed in Slice 10, and now closed as a side effect).
- `lib/ai-classify.js`: added a second verdict, `roleVerdict`, alongside the existing `locationVerdict` — the AI now judges both "is this genuinely tech" and "is this genuinely Memphis," not just the latter.
- `build-listings.js` only asks AI about whichever dimension the rules didn't already resolve — a title the rules already confirmed stays confirmed regardless of what the AI says about it in the same call, and likewise for location. Workday's location-detail lookup (Slice 2) is now only run for jobs that are location-uncertain; a job that's uncertain purely on title (location already a clean match) skips straight to AI.

**A real complication, caught before shipping:** the first working version judged role from title + company + department alone (department is free — already captured at scrape time). It correctly passed Buckman's Digital Innovation Engineer initially... except the department context is what made it *fail* — that posting genuinely sits under Buckman's own **"Marketing"** department (verified directly against the raw page, not a scraper bug), and the AI reasonably read "digital + Marketing" as martech rather than engineering. You pointed out this is a real, common pattern (web/software work sitting under a marketing team responsible for the company site), not an edge case to shrug off. Fix: **title-uncertain jobs now get their actual description fetched** before the AI call, extracted via the same per-ATS logic `build-job-pages.js` already had (pulled out into a new shared `lib/fetch-description.js` so both places use one implementation instead of two copies) and passed as a plain-text excerpt (new `toPlainTextExcerpt()` in `lib/sanitize-description.js`, distinct from the HTML-preserving excerpt already used for the public job-detail page). The system prompt was updated to explicitly weigh real described duties over the title/department when both are available. Re-tested against the same job: correctly passes now, reasoning explicitly through the misleading department (*"Despite the 'Digital Innovation Engineer' title and Marketing department placement, the actual job responsibilities clearly involve software/hardware development work..."*).

**Real before/after, run against live production data:** 25 → 61 listings. Comparing the department-only run against the description-based run directly (both real, both logged) shows this isn't just recall going up — two AutoZone titles ("HR Systems Administrator," "Senior Payroll Systems Analyst") that the department-only pass had wrongly passed got correctly caught and failed once given real description content, netting out against Buckman's fix and landing at the same total both times by coincidence, with materially better precision on both ends. 70 postings reviewed this run, cost $0.11 — confirms the cost concern raised before choosing the narrower `TITLE_MAYBE_PATTERNS` list wasn't the real constraint (Haiku stays cheap even reviewing 10x the previous volume); the real cost was pipeline runtime, which roughly tripled due to the added per-job description fetches (only for title-uncertain jobs, not the full uncertain bucket).

**How to verify yourself:**
1. `node -e 'import("./lib/filter.js").then(({classify}) => console.log(classify({title: "Digital Innovation Engineer", location: "Memphis, Tennessee"})))'` — should show `verdict: "uncertain", titleVerdict: "uncertain"`.
2. `node scripts/build-listings.js` — real run, should take several minutes (description fetches + ~70 AI calls) and print `role:X location:Y` verdicts per reviewed posting; check `data/ai-log.jsonl`'s most recent entry for `jobvite:buckman:olGaAfwv` — should show `"roleVerdict": "pass"`.
3. Confirm the department really is misleading: `curl -s https://jobs.jobvite.com/buckman/jobs/team | grep -B30 'olGaAfwv' | grep 'h2'` — should show `Marketing`, not `Digital`.
