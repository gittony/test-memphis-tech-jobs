// Shared "scrape one Workday employer" logic, used both by the single-
// employer CLI script (scripts/run-workday-employer.js) and the loop that
// runs every configured Workday employer in one go (scripts/run-all-workday.js).
// Never throws — a failure is recorded to data/scraper-health.json and
// returned as { ok: false }, so one broken employer can't stop the others.
...
export async function runWorkdayEmployer(employer) {
  const now = new Date().toISOString();
  const previous = loadScraperHealth(HEALTH_PATH).find((h) => h.employer === employer.key);

  try {
    const { jobs: fresh } = await fetchWorkdayJobs(employer);
