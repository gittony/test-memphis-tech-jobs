// Shared "scrape one UltiPro employer" logic — mirrors
// lib/run-workday-employer.js / lib/run-icims-employer.js / lib/run-oracle-recruiting-employer.js.
...
export async function runUltiproEmployer(employer) {
  const now = new Date().toISOString();
  const previous = loadScraperHealth(HEALTH_PATH).find((h) => h.employer === employer.key);

  try {
    const { jobs: fresh } = await fetchUltiproJobs(employer);
