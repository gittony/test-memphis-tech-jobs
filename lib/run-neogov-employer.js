// Shared "scrape one NEOGOV employer" logic — mirrors
// lib/run-workday-employer.js / lib/run-icims-employer.js /
// lib/run-oracle-recruiting-employer.js. Never throws; a failure is
// recorded to data/scraper-health.json and returned as { ok: false }.

import { fileURLToPath } from "node:url";
import { fetchNeogovJobs } from "./neogov.js";
...
export async function runNeogovEmployer(employer) {
  const now = new Date().toISOString();
  const previous = loadScraperHealth(HEALTH_PATH).find((h) => h.employer === employer.key);

  try {
    const { jobs: fresh } = await fetchNeogovJobs(employer);
