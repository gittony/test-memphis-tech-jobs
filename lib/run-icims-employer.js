// Shared "scrape one iCIMS employer" logic — mirrors lib/run-workday-employer.js.
...
export async function runIcimsEmployer(employer) {
  const now = new Date().toISOString();
  const previous = loadScraperHealth(HEALTH_PATH).find((h) => h.employer === employer.key);

  try {
    const { jobs: fresh } = await fetchIcimsJobs(employer);
